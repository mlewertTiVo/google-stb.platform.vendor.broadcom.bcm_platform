...
uint64_t NxWrap::client() {
   ...
   if (get_nxs(false) != NULL) {
      get_nxs(false)->getNxClient(getpid(), client);
   }
   return client;
}

void NxWrap::regHpdEvt(const sp<INxHpdEvtSrc> &listener) {
   if (get_nxs(false) != NULL) {
      get_nxs(false)->regHpdEvt(listener);
   }
}

void NxWrap::unregHpdEvt(const sp<INxHpdEvtSrc> &listener) {
   if (get_nxs(false) != NULL) {
      get_nxs(false)->unregHpdEvt(listener);
   }
}

void NxWrap::regDspEvt(const sp<INxDspEvtSrc> &listener) {
   if (get_nxs(false) != NULL) {
      get_nxs(false)->regDspEvt(listener);
   }
}

void NxWrap::unregDspEvt(const sp<INxDspEvtSrc> &listener) {
   if (get_nxs(false) != NULL) {
      get_nxs(false)->unregDspEvt(listener);
   }
}
