...
#define NXSERVER_SERVICE_INTERFACE "broadcom.nxserver"

// client singleton for NxServer binder interface
Mutex gLock;
sp<INxServer> gNxServer;
...
const sp<INxServer>& get_nxs(bool retry)
{
   ...
   if (gNxServer.get() == 0) {
      ...
      sp<IBinder> binder;
      do {
         binder = sm->getService(String16(NXSERVER_SERVICE_INTERFACE));
         if (binder != 0) {
            break;
         }
      ...
      if (binder != 0) {
         gNxServer = interface_cast<INxServer>(binder);
      }
      else {
         if (attempt_count >= MAX_ATTEMPT_COUNT) {
            ALOGE("%s: giving up waiting for \'%s\'",
                  __FUNCTION__, NXSERVER_SERVICE_INTERFACE );
         } else if (retry == false) {
