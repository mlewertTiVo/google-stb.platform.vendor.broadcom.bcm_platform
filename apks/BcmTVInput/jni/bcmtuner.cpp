...
//#define LOG_NDEBUG 0
static const char* TAG = "BcmTVInput-JNI";
...
static struct bcmsideband_ctx *gSidebandContext = NULL;
...
Java_com_broadcom_tvinput_BcmTunerJniBridge_releaseSdb(JNIEnv *env, jobject caller) {
    ...
    ALOGV("Release sideband resource.");
    if (gSidebandContext)
        libbcmsideband_release(gSidebandContext);

    gSidebandContext = NULL;
    ALOGD("< releaseSdb()");
    return 0;
}
}
...
Java_com_broadcom_tvinput_BcmTunerJniBridge_initialiseSdb(JNIEnv *env, jobject caller, jobject jsurface) {
    ...
    gSidebandContext = libbcmsideband_init_sideband(0, native_window, &sbGeometryUpdate);
    if (!gSidebandContext)
    {
        ALOGE("%s() failed to obtain a sideband.",__FUNCTION__);
        gSurfaceId = INVALID_HANDLE;
    }

    if (gSidebandContext && gSurfaceClient == NULL) {
