...
Mutex idptLock;
static const sp<IDptHak> BOMX_idpt(void) {
...
void BOMX_InputDataTracker::AddEntry(unsigned pts)
{
    m_ptsTracker.push_back(pts);
    ++m_numInputBuffers;
}

size_t BOMX_InputDataTracker::GetNumEntries()
{
    return m_ptsTracker.size();
}
...
void BOMX_InputDataTracker::SetLastReturnedPts(unsigned pts)
{
    unsigned erasedFrames = 0;

    List<unsigned>::iterator it = m_ptsTracker.begin();
    while ( it != m_ptsTracker.end() ) {
       if ( *it == pts || UNSIGNED_LESS_THAN(*it, pts) ) {
...
unsigned BOMX_InputDataTracker::GetMaxDeltaPts()
{
    unsigned smallest, largest;
    ...
    for (; it != m_ptsTracker.end(); ++it) {
        if (*it != smallest && UNSIGNED_LESS_THAN(*it, smallest) ) {
            smallest = *it;
        } else if (*it != largest && UNSIGNED_LESS_THAN(largest, *it)) {
            largest = *it;
        }
    }
...
void BOMX_InputDataTracker::PrintStats()
{
    if ( m_numInputBuffers > 0 )
        ALOGD("id stats ib:%zu rb:%zu nrb:%zu", m_numInputBuffers, m_numRetBuffers, m_numNotRetBuffers);
}

void BOMX_InputDataTracker::Reset()
{
    m_ptsTracker.clear();
    m_numInputBuffers = 0;
    m_numRetBuffers = 0;
    ...
    m_renderedFrameHandler(this)
{
    ...
    m_numVideoPorts = 0;
    m_pVideoPorts[0] = new BOMX_VideoPort(m_videoPortBase, OMX_DirInput, B_NUM_INPUT_BUFFERS, B_DATA_BUFFER_SIZE_DEFAULT, false, 0, &portDefs, portFormats, numRoles);
    ...
    OMX_IN  OMX_BUFFERHEADERTYPE* pBufferHeader)
{
    ...
    if ( pBufferHeader->nFlags & OMX_BUFFERFLAG_CODECCONFIG )
    {
        ...
        m_inputBuffersTimerId = StartTimer(10,
    ...
    if ( pBufferHeader->nFilledLen > 0 )
    {
        ...
        if ( numRequested > 0 )
        {
            ...
            m_inputDataTracker.AddEntry(pInfo->pts);
        }
        err = m_pVideoPorts[0]->QueueBuffer(pBufferHeader);
        if ( err )
        {
...
OMX_ERRORTYPE BOMX_VideoDecoder::VerifyInputPortBuffers()
{
    ...
    if (portDef.nBufferSize > B_DATA_BUFFER_SIZE_HIGHRES) {
        size_t maxInputBuffMem = B_DATA_BUFFER_SIZE_HIGHRES * B_NUM_INPUT_BUFFERS;
...
{
    CancelTimerId(m_inputBuffersTimerId);
    ReturnInputBuffers(InputReturnMode_eTimeout);
}
...
void BOMX_VideoDecoder::PollDecodedFrames()
{
    ...
    if ( m_pVideoPorts[1]->QueueDepth() > 0 )
    {
                ...
                (m_formatChangeState == FCState_eNone || m_formatChangeState == FCState_eProcessCallback) )
        {
            ...
            if ( NULL != pOmxBuffer )
            {
                ...
                EstimateFrameRate(pHeader->nTimeStamp);
                {
                    ...
                    m_inputDataTracker.SetLastReturnedPts(pBuffer->frameStatus.pts);
                    ReturnInputBuffers();
                    ALOG_ASSERT(queueDepthBefore == (m_pVideoPorts[1]->QueueDepth()+1));

                    if ( m_callbacks.EventHandler && (pHeader->nFlags & OMX_BUFFERFLAG_EOS) != 0 && m_eosDelivered )
                    {
                        (void)m_callbacks.EventHandler((OMX_HANDLETYPE)m_pComponentType, m_pComponentType->pApplicationPrivate, OMX_EventBufferFlag, m_videoPortBase+1, pHeader->nFlags, NULL);
                    }
                }
                if ( destripedSuccess )
                {
        ...
        ALOGV("%s: no client output buffers available", __FUNCTION__);
    }
}
...
void BOMX_VideoDecoder::CopyAlignedSurfaceToClient(uint8_t *pClientMemory, const uint8_t *pSurfaceMemory, uint16_t height, unsigned int pitch, OMX_S32 stride)
{
    if ( pClientMemory && pSurfaceMemory && (unsigned int)stride < pitch )
    {
        for ( uint16_t i=0; i < height; i++ )
        {
            BKNI_Memcpy(pClientMemory, pSurfaceMemory, stride);
            pClientMemory += stride;
            pSurfaceMemory += pitch;
        }
    }
}
...
void BOMX_VideoDecoder::CopySurfaceToClient(const BOMX_VideoDecoderOutputBufferInfo *pInfo)
{
         ...
         pInfo->typeInfo.standard.pClientMemory )
    {
        ...
        bool upAligned = (m_pVideoPorts[1]->GetDefinition()->format.video.nStride < (OMX_S32)surfaceStatus.pitch);
        numBytes = upAligned ? cbOffset : surfaceStatus.height * surfaceStatus.pitch;
        ...
        else
        {
            if ( upAligned )
            {
                CopyAlignedSurfaceToClient(pClientMemory, (uint8_t *)pSurfaceMemory, surfaceStatus.height, surfaceStatus.pitch, m_pVideoPorts[1]->GetDefinition()->format.video.nStride);
            }
            else
            {
        ...
        upAligned = (m_pVideoPorts[1]->GetDefinition()->format.video.nStride/2 < (OMX_S32)surfaceStatus.pitch);
        numBytes = upAligned ? surfaceStatus.height * m_pVideoPorts[1]->GetDefinition()->format.video.nStride/2 : surfaceStatus.height * surfaceStatus.pitch;
        ...
        else
        {
            if ( upAligned )
            {
                CopyAlignedSurfaceToClient(pClientMemory, (uint8_t *)pSurfaceMemory, surfaceStatus.height, surfaceStatus.pitch, m_pVideoPorts[1]->GetDefinition()->format.video.nStride/2);
            }
            else
            {
                BKNI_Memcpy(pClientMemory, pSurfaceMemory, numBytes);
