...
FILE DESCRIPTION
Implementation of an AVI reader.
=============================================================================*/
#include <stdlib.h>
#include <string.h>
   ...
   int64_t *time, VC_CONTAINER_SEEK_FLAGS_T flags, uint64_t *pos)
{
   ...
   VC_CONTAINER_STATUS_T status;
   VC_CONTAINER_TRACK_MODULE_T *track_module;
   int64_t keyframe_offs = INT64_C(0); /* position of last known keyframe */
   AVI_TRACK_CHUNK_STATE_T prev_keyframe_chunk;
   ...
   while((status = STREAM_STATUS(p_ctx)) == VC_CONTAINER_SUCCESS)
   {
      ...
      if (track_num == seek_track_num)
      {
         ...
         if (chunk_flags & AVIIF_KEYFRAME)
            track_module->chunk.flags |= VC_CONTAINER_PACKET_FLAG_KEYFRAME;
         else
            track_module->chunk.flags &= ~(VC_CONTAINER_PACKET_FLAG_KEYFRAME);

         status = VC_CONTAINER_ERROR_NOT_FOUND;
         res = avi_compare_seek_time(track_module->chunk.time_pos, *time, chunk_flags & AVIIF_KEYFRAME, flags);
                                              ...
                                              VC_CONTAINER_PACKET_T *p_packet, uint32_t flags )
{
   ...
   if (p_state->chunk_data_left == 0)
   {
         ...
         p_ctx->tracks[track_num]->priv->module->chunk.state != &module->state)
      {
         ...
         vc_container_assert(((p_state->data_offset + p_state->chunk_data_left + 1) & ~1) == next_chunk);
   ...
   VC_CONTAINER_SEEK_MODE_T mode, VC_CONTAINER_SEEK_FLAGS_T flags)
{
   ...
   /* Make the first track within the stream the global offset */
   first = 0;
   for(i = 1; i < p_ctx->tracks_num; i++)
   {
      if (p_ctx->tracks[i]->priv->module->chunk.local_state.data_offset <
          p_ctx->tracks[first]->priv->module->chunk.local_state.data_offset)
         first = i;
   }
   p_ctx->priv->module->state.data_offset = p_ctx->tracks[first]->priv->module->chunk.local_state.data_offset;
   p_ctx->tracks[first]->priv->module->chunk.state = &p_ctx->priv->module->state;
