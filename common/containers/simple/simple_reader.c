...
FILE DESCRIPTION
Implementation of reader for the simple container format.

For a quick description of this format, see simple_common.h.
=============================================================================*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static VC_CONTAINER_STATUS_T simple_read_header( VC_CONTAINER_T *ctx )
{
          ...
          simple_read_line(ctx) == VC_CONTAINER_SUCCESS)
   {
      ...
      if (!memcmp(module->line, "TRACK ", sizeof("TRACK ")-1))
      {
         ...
         {
            LOG_ERROR(ctx, "too many tracks, ignoring TRACK %4.4s", (char *)&fourcc);
            continue;
         }
