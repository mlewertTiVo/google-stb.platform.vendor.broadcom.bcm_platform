/*=============================================================================
Copyright (c) 2010-2011 Broadcom Europe Limited.
All rights reserved.

Project  :  Containers Library
Module   :  Containers I/O
File     :  $RCSfile: containers_io.c,v $
Revision :  $Revision: #0 $

FILE DESCRIPTION
Containers I/O common code.
=============================================================================*/
...
#ifdef _VIDEOCORE
      dma_memcpy(data + read, cache->buffer + cache->position, bytes);
#else
   ...
   VC_CONTAINER_IO_PRIVATE_CACHE_T *cache, const uint8_t *data, size_t size )
{
   ...
   while(size)
   {
      ...
      else
      {
         /* Write in the cache */
#ifdef _VIDEOCORE
         dma_memcpy(cache->buffer + cache->position, data + written, bytes);
#else
...
static void *async_io_thread(VOID *argv)
{
   ...
   int ret = filesys_register();
   vc_container_assert(!ret);
