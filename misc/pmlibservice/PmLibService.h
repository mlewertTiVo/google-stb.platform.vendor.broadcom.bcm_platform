...
{
public:
    status_t             setState(struct pmlib_state_t *state);
    status_t             getState(struct pmlib_state_t *state);

                         PmLibService();
    virtual              ~PmLibService();

private:

    static Mutex         mLock;
    void *               mPmCtx;
    struct pmlib_state_t mState;
